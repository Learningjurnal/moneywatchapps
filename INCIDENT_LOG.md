...
## 2026-09-19 — Market Heatmap masih hanya LQ45: FS_RD top-60-by-cap sampling diganti whole-market unified-screener (dashboard widget + halaman Heatmap penuh)

- **Laporan user:** "potofolio snapshot - command center - 4Opportunity Radar belum sinkron dengan screener, Market Heatmap masih menampilkan hanya lq45, tidak sesuai dengan aturan, seharunys seluruh saham dan hanya memfilter yang masuk kriteria."
- **Root cause "Market Heatmap":** widget dashboard `renderDashboardHeatmapPreview()` (04-render.js) dan halaman penuh "Heatmap" (`fsRenderHeatmap()`, 07-flowscan.js, dibuka lewat tombol "Lihat Semua →" dari widget yang sama) SAMA-SAMA membaca `FS_RD` (07-flowscan.js). `FS_RD` diisi oleh `fsInit()` dengan **top-60-by-market-cap** dari `FS_UNIV` (~958 ticker) via `FS_UNIV.slice().sort(cap desc).slice(0,60)` lalu dihitung client-side lewat `fsGenData()`/`fsProcess()` — TIDAK PERNAH menyentuh endpoint whole-market Invezgo. Ini pelanggaran CLAUDE.md aturan #2 (sample kecil hardcoded/terpotong, bukan seluruh ~958 emiten BEI difilter kriteria) — sekelas insiden Screener LQ45 dan Bandarmology market-aggregate view yang sudah pernah dicatat sebelumnya. Perkataan user "hanya lq45" sedikit tidak presisi (sample-nya top-60-by-cap, bukan literal daftar LQ45), tapi substansinya sama persis: sample kecil, bukan seluruh bursa.
- **Root cause "Opportunity Radar belum sinkron dengan screener" — DUA fitur "Opportunity Radar" berbeda ternyata ada di app ini:**
  1. Widget dashboard "Opportunity Radar" (Command Center) — SUDAH diperbaiki SEBELUM sesi ini (commit `46ad3b8`/PR #228, sudah di `main`): sekarang membaca `GET /api/idx/unified-screener` dengan sort default yang sama dengan halaman Screener (`goPage('radar')` route ke `renderUnifiedScreenerPage()` yang SAMA — dikonfirmasi lewat pembacaan router `06-analysis-router.js`). Tidak ditemukan bug tersisa di jalur ini lewat pembacaan kode.
  2. **Halaman "Opportunity Radar" TERPISAH di Command Center** (`26-commandcenter.js`, `page-radar`, `RADAR_STATE`, `GET /api/idx/opportunity-radar`, formula **Margin-of-Safety/ROE/PE ("Radar Composite Score")** — BUKAN formula Whale/Uptrend milik `generateUnifiedScreener()`) — fitur ini MASIH HIDUP, sub-tab "Universe Screener (950+)"-nya menghitung & menampilkan skor whole-market yang BEDA dan bisa TIDAK SEPAKAT dengan urutan/label halaman Screener (`48-unified-screener.js`) untuk saham yang sama. Inilah kemungkinan besar sumber laporan user "belum sinkron dengan screener" — dua fitur bernama mirip ("Opportunity Radar" vs "Screener"), sama-sama whole-market, formula beda, hasil bisa beda untuk ticker yang sama.
  - **Belum diperbaiki di sesi ini** — perlu keputusan user dulu: gabungkan sub-tab "Universe Screener" milik halaman Opportunity Radar ke Screener (pola konsolidasi yang sama dengan PR #228, hapus duplikasi formula), atau biarkan terpisah tapi diberi label yang jelas membedakan ("MoS/ROE Radar" vs "Whale/Uptrend Screener") supaya perbedaan hasil tidak terbaca sebagai bug. 3 sub-tab lain milik halaman ini (Anomaly Structural & ARA, Visualisasi Alur Transaksi, Kalender Aksi Korporasi) BUKAN duplikasi — fitur real yang berbeda, tidak disentuh.
- **Perbaikan (scope: Market Heatmap saja, dashboard widget + halaman Heatmap penuh; TIDAK menyentuh `fsRunScanner()`/`page-scanner` — fitur beda, sudah orphan/unreachable dari nav manapun per audit menu sebelumnya, dan TIDAK menyentuh Factor Heatmap/`fhmRender()` di halaman yang sama — sumber data `QT.scData`/`scBuildSim()` terpisah, kemungkinan simulasi & scoped LQ45 sendiri, dicatat sebagai temuan terpisah untuk sesi berikutnya, bukan bagian dari laporan user kali ini):**
  - `public/js/07-flowscan.js`: fungsi baru `fsFetchUnifiedHeatmapData()` — fetch `GET /api/idx/unified-screener` (whole-market, sama seperti Screener), filter kriteria `whaleDataAvailable` (saham yang benar-benar punya sinyal/data teknikal hari ini — bukan sample market-cap), urutkan berdasarkan `|whaleScore|` terbesar, cache 60 detik (dipakai bareng oleh widget dashboard & halaman penuh). `fsRenderHeatmap()` diubah jadi `async`, menerima param `force` untuk tombol Refresh baru, tampilkan catatan jujur "Menampilkan N dari M saham bersinyal (dari seluruh X emiten BEI)".
  - `public/js/04-render.js`: `renderDashboardHeatmapPreview()` diubah jadi `async`, memanggil `fsFetchUnifiedHeatmapData()` yang sama, ambil top-10 by `|whaleScore|`.
  - `public/index.html`: dropdown mode "Skor/CMF/Chg%" di halaman Heatmap dihapus (field `cl`/`chgPct` milik `fsProcess()` lama sudah tidak ada di sumber data baru) — diganti label statis "Whale Score" + tombol Refresh. Subtitle halaman diperbarui supaya tidak lagi salah menyebut "faktor fundamental saham LQ45" untuk KESELURUHAN halaman (sekarang membedakan Flow Heatmap [whole-market] vs Factor Heatmap [cakupan terbatas, belum diperbaiki]). Cache-bust `04-render.js?v=20260919d`, `07-flowscan.js?v=20260919d`.
- **Test regresi (`test_suite.js`):** test lama `REGRESSION GUARD: Market Heatmap preview must keep the real-vs-simulated disclosure marker` (mengharuskan baca `.simulated`/`fsSrcDot()` dari FS_RD — sudah tidak relevan karena sumber data baru tidak pernah fabrikasi/tidak butuh marker SIM) diganti test baru yang mengecek: kedua fungsi (`renderDashboardHeatmapPreview()`, `fsRenderHeatmap()`) memanggil `fsFetchUnifiedHeatmapData()` dan TIDAK LAGI membaca `FS_RD`; `fsFetchUnifiedHeatmapData()` benar memanggil endpoint whole-market & memfilter `whaleDataAvailable`. Dikonfirmasi **FAIL tanpa fix** (`git stash` hanya `04-render.js`+`07-flowscan.js`, test baru sudah ada → error "renderDashboardHeatmapPreview() body not found (must be async now)") → **PASS dengan fix dikembalikan** (237/237 lulus semua suite).
- **Verifikasi:** `node -c` semua file yang diubah, `npm test` 237/237 lulus, `npm run lint` bersih.
- **Batasan yang tidak bisa diuji di sandbox ini:** sandbox tanpa akses jaringan ke `moneywatchapps.vercel.app`/Invezgo — perilaku live endpoint `/api/idx/unified-screener` dengan data pasar riil (jumlah saham `whaleDataAvailable` hari itu, dsb.) tidak bisa diverifikasi langsung, hanya lewat pembacaan kode `generateUnifiedScreener()` (sudah teruji & dipakai halaman Screener yang sama). User perlu konfirmasi visual setelah deploy: buka dashboard, cek widget "Market Heatmap" menampilkan campuran ticker (bukan cuma bluechip LQ45 seperti BBCA/BBRI/TLKM terus-menerus), lalu klik "Lihat Semua →" dan pastikan halaman Heatmap penuh menampilkan jumlah saham yang jauh lebih banyak dari 60.
- **Update — user memilih "Gabung ke Screener" (AskUserQuestion):** sub-tab "Universe Screener (950+)" milik halaman "Opportunity Radar" terpisah (26-commandcenter.js, formula MoS/ROE/PE) DIHAPUS. Yang ikut dihapus karena hanya dipakai sub-tab itu: `renderRadarScreenerSubTab()`, `getOpportunityRadarItems()`, `loadOpportunityRadarUniverse()`, `updateRadarFilter()`, dan field `RADAR_STATE.items/summary/search/index/sector/zone/flow/sort/order/page/pageSize/isLoading/lastUpdated` (termasuk 2 `window.*` export yang jadi menggantung). Default `RADAR_STATE.activeTab` diganti dari `'screener'` ke `'anomaly-ara'`. Banner ringkasan (Total Universe/Buy Zone/Aksi Korporasi/Data Terbatas) di header halaman ikut dihapus karena datanya berasal dari fetch yang sama yang dihapus. Tombol header diganti "Screener (950+) →" (`onclick="goPage('radar')"`, ke halaman Screener real) menggantikan tombol Refresh universe lama. 3 sub-tab lain (Anomaly Structural & ARA, Visualisasi Alur Transaksi, Kalender Aksi Korporasi) TIDAK disentuh — tetap dipakai lewat drill-down "Alur Transaksi" dari Bandarmology/Stock Intel (`fsOpenBrokerFlowTicker()` di 07-flowscan.js → `goPage('radar')` + `setRadarSubTab('flow-trail')`). Server route `GET /api/idx/opportunity-radar` (`getUniverseOpportunityRadar()`) SENGAJA TIDAK dihapus — tidak ada lagi caller client-side, tapi backend-nya tetap benar & tidak berbahaya dibiarkan (bisa dipakai lagi kalau ada kebutuhan API terpisah nanti); dicatat sebagai kandidat cleanup terpisah kalau memang mau dihapus total. Test regresi baru: memastikan `renderRadarScreenerSubTab()`/tombol "Universe Screener (950+)"/`getOpportunityRadarItems()`/`loadOpportunityRadarUniverse()` tidak ada lagi, 3 sub-tab real tetap ada, dan tombol "Screener (950+) →" ada. Dikonfirmasi FAIL tanpa fix → PASS dengan fix (238/238 lulus semua suite). `node -c`, `npm test`, `npm run lint` semua bersih. Cache-bust `26-commandcenter.js?v=20260919a`.
- **Update 2 — user: "dihapus diganti sectroal heatmap"** (soal temuan terpisah yang dicatat di atas: tab "Factor Heatmap" di halaman Heatmap masih memakai `QT.scData`/`scBuildSim()`, scoped ke pilihan universe Quant Screener). Instruksi: hapus tab "Factor Heatmap", ganti dengan "Heatmap Sektoral" — fitur yang SUDAH ADA & SUDAH real (task session sebelumnya: "Heatmap Sektoral: ganti simulasi jadi data Invezgo real"), yaitu mode "sector" milik Smart Money Screener (`fsRenderSectorHeatmapMode()`, 07-flowscan.js) — agregasi arus dana broker real per sektor dari `GET /api/idx/accumulation-distribution` (whole-market, via `FS_BROKER_SCAN` yang sama dipakai mode "Broker Flow Riil").
  - **Perbaikan:**
    - `07-flowscan.js`: `fsRenderSectorHeatmapMode()` diubah menerima parameter opsional `targetId` (default `'sms-sector-content'`, mode lama tidak berubah perilakunya) supaya bisa dirender ke container DOM berbeda tanpa duplikasi logika fetch/agregasi. `hmSwitchTab()`: tab kedua diganti dari `'factor'` jadi `'sector'`, memanggil `fsRenderSectorHeatmapMode('hm-sector-content')` menggantikan `fhmRender()`.
    - `public/index.html`: tombol tab "Factor Heatmap" diganti "Heatmap Sektoral"; markup lama (`fhm-factor`/`fhm-sort` select, `fhm-grid`, canvas `fhm-dist-chart`) dihapus total, diganti satu container kosong `id="hm-sector-content"` (diisi JS). Judul & subtitle halaman diperbarui ("Heatmap & Factor Analysis" → "Heatmap"; subtitle sekarang menyebut kedua tab yang benar-benar ada: Flow Heatmap & Heatmap Sektoral, keduanya whole-market).
    - `11-quant.js`: `fhmRender()` dihapus total (bukan cuma di-disconnect) — sudah tidak ada pemanggil manapun setelah tombol tabnya hilang (hook `goPage('factor-heatmap')` yang lama juga sudah dead code sebelum perubahan ini, tidak pernah dipanggil `goPage()` manapun di app). `QT.scData`/`scBuildSim()` TIDAK disentuh — masih dipakai halaman Screener (Quant) yang real, cuma konsumen `fhmRender()`-nya yang hilang.
    - `13-realdata.js`: 1 komentar diperbarui (tidak lagi menyebut "Factor Heatmap" sebagai salah satu konsumen `QT.scData`).
  - **Test regresi (`test_suite.js`):** memastikan `fhmRender()`/hook `'factor-heatmap'` tidak ada lagi di `11-quant.js`; `fsRenderSectorHeatmapMode()` menerima param `targetId`; `hmSwitchTab()` memanggil `fsRenderSectorHeatmapMode('hm-sector-content')` untuk tab sector dan TIDAK memanggil `fhmRender()`; markup `hm-sector-content` ada di index.html sementara `fhm-grid`/`fhm-dist-chart`/teks tombol "Factor Heatmap" sudah tidak ada. Dikonfirmasi FAIL tanpa fix (`git stash` semua file terkait) → PASS dengan fix (239/239 lulus semua suite).
  - **Verifikasi:** `node -c` semua file yang diubah, `npm test` 239/239 lulus, `npm run lint` bersih. Cache-bust: `07-flowscan.js?v=20260919e`, `11-quant.js?v=20260919b`, `13-realdata.js?v=20260919a`.
  - **Batasan yang tidak bisa diuji di sandbox ini:** perilaku live `fsRenderSectorHeatmapMode()` dengan data Invezgo sungguhan (sandbox tanpa akses jaringan ke `api.invezgo.com`) — fungsinya sendiri sudah teruji/dipakai produksi sebagai mode "sector" Smart Money Screener sebelum sesi ini, jadi ini murni penggunaan ulang (reuse), bukan kode baru yang belum pernah tervalidasi. User perlu konfirmasi visual setelah deploy: buka halaman Heatmap, klik tab "Heatmap Sektoral", pastikan menampilkan agregasi per-sektor + tabel ranking (bukan lagi grid RSI/Momentum per saham).
- **Koreksi user setelah deploy — SALAH EDIT ditemukan lewat screenshot:** widget dashboard "Market Heatmap" (bukan halaman Heatmap di atas — widget di Portofolio Snapshot/Command Center) sudah dibetulkan ke whole-market (`fsFetchUnifiedHeatmapData()`, Whale Score per-saham) pada entri sebelumnya, TAPI user mengoreksi: "anda salah edit, market heat map seharusnya diganti sectoral heat map, lihat Sectoral Insight, data seharusnya diambil dari situ" (disertai screenshot widget menampilkan kartu per-saham AYLS/ASLI/CASH/dst dengan skor "+1"). Instruksi eksplisit: widget ini harus jadi heatmap SEKTORAL, dan datanya harus diambil dari halaman "Sector Insight" (44-sectoral-insight.js) yang sudah ada — bukan dari `generateUnifiedScreener()`'s per-saham whaleScore yang dipakai sebelumnya.
  - **Perbaikan:**
    - `public/js/44-sectoral-insight.js`: tambah `window.siGetSectorHeatmapData()` — wrapper tipis yang memanggil `siComputeAllSectors('1D')`, fungsi PERSIS yang dipakai halaman Sector Insight sendiri untuk menghitung 11 sektor resmi IDX (CMF-konstituen big-cap real, sudah terurut Akumulasi->Distribusi) — bukan duplikasi logika, murni ekspos fungsi privat modul (sebelumnya tidak ada di `window.*`) ke luar.
    - `public/js/04-render.js`: `renderDashboardHeatmapPreview()` ditulis ulang total — bukan lagi `async`/fetch (tidak perlu, `siComputeAllSectors()` sinkron), memanggil `window.siGetSectorHeatmapData()`, render 11 kartu sektor (nama, CMF% berwarna, label status seperti "Akumulasi Masif"/"Distribusi Ringan"). "Lihat Semua →" diganti dari `goPage('heatmap')` (halaman Heatmap yang barusan diperbaiki di atas — SEKARANG DISADARI: sumber datanya beda, per-saham/broker-flow, bukan Sector Insight) ke `goPage('sectoral-insight')` — supaya preview selalu konsisten dengan halaman yang benar-benar jadi sumber datanya (kelas bug yang sama persis dengan insiden Opportunity Radar sebelumnya, ditangkap sebelum sempat jadi laporan user berikutnya).
    - `public/index.html`: judul kartu dashboard diganti dari "Market Heatmap" jadi "Heatmap Sektoral" (menghindari kebingungan lebih lanjut dengan halaman Heatmap yang formulanya beda), grid diperlebar (`minmax(80px,1fr)`->`minmax(110px,1fr)`) untuk label sektor yang lebih panjang dari kode ticker.
  - **Test regresi (`test_suite.js`):** test lama untuk `renderDashboardHeatmapPreview()` (yang mengharuskan `fsFetchUnifiedHeatmapData`) DIHAPUS dari test gabungan, diganti test baru: `siGetSectorHeatmapData()` ada & delegasi ke `siComputeAllSectors()`; `renderDashboardHeatmapPreview()` memanggil `siGetSectorHeatmapData()` dan TIDAK LAGI memanggil `fsFetchUnifiedHeatmapData`/`FS_RD`; tombol "Lihat Semua" mengarah ke `goPage('sectoral-insight')`; judul kartu "Heatmap Sektoral" ada di index.html. Test untuk `fsRenderHeatmap()` (halaman Heatmap terpisah, TIDAK berubah oleh koreksi ini) dipecah jadi test sendiri supaya tidak ikut campur. Dikonfirmasi FAIL tanpa fix (`git stash` 3 file terkait) → PASS dengan fix (240/240 lulus semua suite).
  - **Verifikasi:** `node -c` semua file, `npm test` 240/240 lulus, `npm run lint` bersih. Cache-bust: `04-render.js?v=20260919e`, `44-sectoral-insight.js?v=20260919a`.
  - **Pelajaran untuk sesi ini:** "Market Heatmap" dan "Heatmap Sektoral" adalah 2 istilah yang terdengar hampir sama tapi merujuk ke 2 halaman/fitur BERBEDA (halaman "Heatmap" terpisah vs halaman "Sector Insight") — instruksi user "ganti X jadi sectoral heatmap" sebelumnya salah diinterpretasikan sebagai "buat versi sektoral dari formula whaleScore yang sudah ada" alih-alih "pakai data dari fitur Sectoral/Sector Insight yang sudah ada". Kalau nama fitur mirip tapi user secara eksplisit menyebut "lihat [halaman lain]", verifikasi dulu halaman itu dan reuse datanya, jangan asumsikan formula baru yang mirip secara konsep sudah cukup.

## 2026-09-20 — "Berita Pasar & Katalis Terkoneksi" kosong: OpenRouter dijadikan provider utama (dibalik dari Claude), Invezgo dikonfirmasi TIDAK ADA endpoint berita

- **Laporan user:** "Berita Pasar & Katalis Terkoneksi Terkoneksi, belum menampilkan apa apa, dimana invezgo sudah ada API news feed dan sudah API key".
- **Investigasi:** card ini (halaman Sector Insight, `44-sectoral-insight.js`'s `siFetchNews()`) memanggil `GET /api/sectoral-news` (server.js) — SATU-SATUNYA sumber datanya adalah Claude (Anthropic) via `web_search` tool. Karena Anthropic API key app ini belum ada credit-nya (sudah dicatat sesi sebelumnya), panggilan ini SELALU gagal → card selalu kosong dengan honest-fallback message, bukan bug tampilan.
- **Soal klaim "Invezgo sudah ada API news feed":** dicek dari OpenAPI spec resmi Invezgo (`invezgo-openapi-full.json`) dan audit endpoint sebelumnya (`AUDIT_INVEZGO_UNUSED_ENDPOINTS_2026-09-18.md`) — ditemukan kandidat `posts/category/{category}` dan `posts/space/category/{code}/NEWS`, TAPI skema responsnya tidak pernah terverifikasi (OpenAPI spec-nya sendiri kosong untuk `responses.200`, tidak ada contoh field). Ditanyakan ke user untuk sample respons real sebelum menulis parser (sesuai aturan #1/#3) — **user mengonfirmasi langsung: Invezgo ternyata TIDAK ADA fitur berita.** Klaim awal user salah/salah paham — bukan bug di kode, endpoint yang dimaksud memang tidak eksis. Tidak ada perubahan kode terkait Invezgo untuk temuan ini.
- **Keputusan user:** "untuk news pakai API 9router (OpenRouter) sebagai utama dan anthropic sebagai backup, agar news tidak kosong saat credit habis" — kebalikan dari urutan provider default app ini (Claude utama, OpenRouter cadangan, dipakai di `/api/ai/agent-chat`), KHUSUS untuk endpoint ini saja, karena root cause "kosong terus"-nya justru Claude yang selalu gagal duluan (credit habis) sebelum sempat ada fallback.
- **Perbaikan (`server.js`, `/api/sectoral-news`):**
  1. Prompt & parser JSON diekstrak jadi 2 fungsi bersama (`buildSectoralNewsPrompt()`, `extractSectoralNewsJsonArray()`) — dipakai KEDUA provider, supaya format/kualitas hasil tidak berbeda tergantung jalur mana yang berhasil.
  2. Fungsi baru `fetchSectoralNewsViaOpenRouter(orConfig)` — request ke `openrouter.ai/api/v1/chat/completions` dengan `plugins:[{id:'web', max_results:8}]` (fitur "web plugin" OpenRouter supaya model benar-benar mencari data web, BUKAN menjawab dari memori training — kalau tidak pakai ini, risikonya model bisa MENGARANG judul/URL berita yang terlihat valid, pelanggaran zero-fabricated-data yang lebih parah daripada card kosong).
  3. Urutan panggilan di route dibalik: **OpenRouter dicoba DULU** (kalau `getOpenRouterConfig()` ada) → kalau gagal/tidak terkonfigurasi, baru fallback ke Claude (kode lama, sekarang jadi cadangan) → kalau dua-duanya gagal, honest-empty seperti sebelumnya. Flag `resolved` mencegah Claude tetap dipanggil (buang-buang kuota berbayar) kalau OpenRouter sudah berhasil.
- **Test regresi (`test_suite.js`):** cek `fetchSectoralNewsViaOpenRouter()` ada & memakai `plugins:[{id:'web'...}]`; urutan pemanggilan di route (`fetchSectoralNewsViaOpenRouter` sebelum `callClaudeWithRetry`, posisi string diperiksa, sama seperti pola provider-order check yang sudah ada untuk `/api/ai/agent-chat`); Claude fallback digerbangi flag `resolved`; route TIDAK mengandung referensi Invezgo apa pun (menjaga keputusan user "Invezgo tidak dipakai" tidak diam-diam berubah lagi nanti). Dikonfirmasi FAIL tanpa fix (`git stash server.js`) → PASS dengan fix (241/241 lulus semua suite).
