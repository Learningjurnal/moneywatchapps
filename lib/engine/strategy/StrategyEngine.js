...
// FIX (2026-09-25, user-reported: "Belum ada saham STRONG/QUALIFIED untuk
// 2026-09-25" every day since the 4-strategy cron schedule went live):
// this used to be a single global key shared by ALL strategies. That was
// silently correct back when only swing-flow ever ran daily (a shared
// cursor IS a per-strategy cursor when there's only one strategy calling
// it), but once 4 separate schedule entries were added (one per
// strategy, 2026-09-24), every strategy's run advanced the SAME cursor
// position — so each strategy's daily ~9-11-ticker batch scanned whatever
// slice of the universe the PREVIOUS strategy's run happened to leave
// off at, not a slice specific to itself. Net effect: a ticker checked
// today for swing-flow was never checked again for day-trading at that
// same position — each strategy's effective coverage of the 958-ticker
// universe was ~4x slower than intended, compounding the "small sample,
// zero hits some days" symptom this session already expected (score
// bands require >=70/100, a genuinely high bar) into an even smaller
// effective sample per strategy.
function warmCursorKey(strategyId) {
  return `strategy_engine:warm:cursor:${strategyId}`;
}

// FIX (2026-09-25, follow-up to removing FOREIGN from weights/
// mandatoryConditions — see foreignFlow.js's header comment for the
// incident): no historical order-book/intraday data exists to backtest
// Strategy Engine V1 against, so the only honest way to confirm the fix
// actually raised hidden-accumulation/momentum-candidate's near-zero
// STRONG/QUALIFIED rate is to watch it happen for real, going forward.
// warmStrategyEngineRotating() already logged WHICH tickers qualified
// (signalLogKey) but never how many were scanned or what every OTHER
// ticker's status was — so there was no way to compute a qualifying
// RATE, only a raw count that says nothing without knowing the
// denominator. dailyStatsKey() adds that: a per-strategy, per-day
// {processed, STRONG, QUALIFIED, WATCH, REJECT, DATA_INSUFFICIENT}
// counter, accumulated across however many rotating runs happen that
// day, same TTL/retention as the signal log.
function dailyStatsKey(strategyId, dateKey) {
  return `strategy_engine:daily_stats:${strategyId}:${dateKey}`;
}
...
async function warmStrategyEngineRotating(timeBudgetMs, strategyId, batchSize) {
  ...
  const cursorBefore = Number((await storeGet(warmCursorKey(strategyId))) || 0);
  let idx = total > 0 ? cursorBefore % total : 0;
  let processed = 0;
  let attempts = 0;
  const signals = [];
  const statusCounts = { STRONG: 0, QUALIFIED: 0, WATCH: 0, REJECT: 0, DATA_INSUFFICIENT: 0 };
  ...
  while (total > 0 && attempts < total && (Date.now() - start) < budget) {
    ...
    processed++;
    if (Object.prototype.hasOwnProperty.call(statusCounts, result.status)) statusCounts[result.status]++;
  ...
  await storeSetEx(warmCursorKey(strategyId), idx, 0);
  ...
  if (processed > 0) {
    const statsKey = dailyStatsKey(strategyId, dateKey);
    const existingStats = (await storeGet(statsKey)) || { processed: 0, STRONG: 0, QUALIFIED: 0, WATCH: 0, REJECT: 0, DATA_INSUFFICIENT: 0 };
    existingStats.processed = (existingStats.processed || 0) + processed;
    Object.keys(statusCounts).forEach(k => { existingStats[k] = (existingStats[k] || 0) + statusCounts[k]; });
    await storeSetEx(statsKey, existingStats, STRATEGY_SIGNAL_LOG_TTL_SEC);
  }
...
/**
 * getStrategyEngineDailyStats(strategyId, days?) — read-only, for
 * validating a scoring-logic change (e.g. the FOREIGN indicator removal)
 * against real signal-generation behavior over time instead of a
 * historical backtest that isn't possible here (Strategy Engine V1's
 * indicators need order-book/intraday snapshots Invezgo has no confirmed
 * historical retention for). Returns each of the last `days` days'
 * accumulated {processed, STRONG, QUALIFIED, WATCH, REJECT,
 * DATA_INSUFFICIENT} counts from warmStrategyEngineRotating(), oldest
 * first — a day with no cron activity simply reads all-zero, not an
 * error. `qualifyingRate` (STRONG+QUALIFIED / processed, null if
 * processed is 0) is computed here rather than stored, so it's always
 * consistent with the raw counts even if this function's formula changes
 * later.
 */
async function getStrategyEngineDailyStats(strategyId, days = 14) {
  ...
  for (let i = boundedDays - 1; i >= 0; i--) {
    ...
    const stored = await storeGet(dailyStatsKey(strategyId, dateKey));
    const stats = stored || { processed: 0, STRONG: 0, QUALIFIED: 0, WATCH: 0, REJECT: 0, DATA_INSUFFICIENT: 0 };
    const qualifying = stats.STRONG + stats.QUALIFIED;
    out.push({
      date: dateKey,
      ...
      qualifyingRate: stats.processed > 0 ? Math.round((qualifying / stats.processed) * 1000) / 10 : null
    });
  }
  return { strategyId, days: out };
}
...
export { runStrategyForTicker, runStrategyForUniverse, warmStrategyEngineRotating, getLatestStrategyEngineSignals, getDailyTopPicks, getStrategyEngineDailyStats };
