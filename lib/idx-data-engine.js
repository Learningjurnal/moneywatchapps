...
async function generateBandarMovementData(ticker, options = {}) {
  ...
  if (allBuyers.length > 0 && allSellers.length > 0) {
    ...
    // FIX (2026-09-26, user-reported design review): flowVal here is NOT an
    // observed buyer-to-seller trade — IDX broker summaries only disclose
    // each broker's aggregate buy/sell value for the ticker, never which
    // broker traded with which counterparty. This proportionally allocates
    // each seller's value across buyers by their share of total buy value
    // (and vice versa), which is a reasonable estimate of the distribution
    // but not a real pairwise observation. Kept as an estimate (user's
    // explicit choice over dropping pairwise flow entirely), so `isEstimate`
    // is exposed for the UI to disclose honestly instead of presenting
    // these as measured flows — see bmRenderBrokerDistributionSankey() in
    // public/js/49-bandar-movement.js.
    const links = [];
