...
async function generateBrokerSummary(ticker, quoteData, timeframe = '1D', investor = 'all', market = 'RG') {
  ...
  const invezgo = await fetchInvezgoBrokerSummary(clean, fromDate, toDate, investor, market);
  if (invezgo.ok) {
...
async function generateBandarMovementData(ticker, options = {}) {
  ...
  const { date, timeframe = '1D', isBigMoney = false, investor = 'all', market = 'RG' } = options;

  // 1. Broker Summary (also supplies data for the Sankey Distribution flow)
  const brokerSummary = await generateBrokerSummary(clean, null, timeframe, investor, market);
