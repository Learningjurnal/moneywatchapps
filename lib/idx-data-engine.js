...
import { getOrFetch, invezgoFetch, makeRequestId, freshnessSecondsFrom } from './invezgo-client.js';
...
async function generateExitHypothesis(ticker, position) {
  ...
  // Data Quality Gate — Stage 1 / non-blocking (roadmap B.5, item "Temuan
  // 1"). generateTradingHypothesis() already refuses to output a fabricated
  // BUY when dataQuality.status !== REAL (see gateFailures above); this exit
  // side had no equivalent at all — a SELL/HOLD call on an already-open
  // position could be (and still can be, at this stage) generated from
  // STALE/UNAVAILABLE/SIMULATION-quality data with nothing surfacing that
  // fact anywhere. This does NOT flip `side` or suppress the recommendation
  // — doing that immediately would silently withhold exit warnings (e.g. a
  // real SL breach) the moment a feed is merely stale, which is its own
  // failure mode. It only makes the degradation visible so the caller/UI
  // can warn instead of presenting the call as equally trustworthy as one
  // computed from REAL data. Promote this to an actual block once this
  // telemetry has been observed safe, mirroring the MW-P0-001 Stage 1
  // rollout precedent.
  const dataQualityWarning = dataQuality.status !== DATA_QUALITY_STATUS.REAL
...
// Broker Summary cache TTL per audit doc `caching.ttl_sec.broker_summary`.
const INVEZGO_BROKER_SUMMARY_CACHE_TTL_SEC = 300;
...
async function fetchInvezgoBrokerSummary(ticker, fromDate, toDate) {
  ...
  // handled by invezgo-client.js — never call fetch() directly here (see
  // lib/invezgo-client.js header comment for why: this app is serverless,
  // an in-process counter can't enforce the 30k/month budget safely).
  ...
  const envelope = await getOrFetch('broker_summary', params, INVEZGO_BROKER_SUMMARY_CACHE_TTL_SEC, async () => {
    try {
      ...
      // Contract check per INV-P1-002: schema is not yet verified against a
      // live response. Rather than guess-mapping an unrecognized shape into
      // fabricated buyers/sellers, fail closed to INVALID — this must never
      // silently become UNAVAILABLE-and-forgotten, it should surface as a
      // schema-mismatch alert (see observability.alerts.schema_validation_failure).
      ...
      const dataTimestamp = d.date || d.timestamp || d.report_date || retrievedAt;
      return {
        status: 'REAL',
        source: 'invezgo_api',
        provider: 'Invezgo',
