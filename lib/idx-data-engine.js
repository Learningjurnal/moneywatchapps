...
async function computeStockSignal(ticker) {
  ...
  if (!price || !tech) {
    return {
      ...
      dataQuality, error: !price ? (quote?.isSimulated ? 'Quote is simulated (Yahoo unreachable, no cache) — refusing to signal on fabricated price' : 'No live price') : 'Insufficient price history (need 30+ daily bars)'
    };
  }
