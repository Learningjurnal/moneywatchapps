...
async function computeConfluence(ticker) {
  ...
  if (brokerSummary && brokerSummary.isValidTicker && !brokerSummary.isSimulated) {
  ...
  const trendEv = evidence.find(e => e.group === 'TREND');
  const regimeEv = evidence.find(e => e.group === 'MARKET_REGIME');
  if (trendEv && regimeEv && trendEv.direction !== regimeEv.direction && trendEv.direction !== 'NEUTRAL' && regimeEv.direction !== 'NEUTRAL') {
    contradictions.push(`Teknikal individual ${trendEv.direction === 'BULLISH' ? 'bullish' : 'bearish'} tapi regime pasar IHSG sedang ${regimeEv.direction === 'BULLISH' ? 'bullish' : 'bearish'} — sinyal berpotensi melawan arus pasar.`);
  }
  const momEv = evidence.find(e => e.group === 'MOMENTUM');
  if (trendEv && momEv && trendEv.direction !== momEv.direction && trendEv.direction !== 'NEUTRAL' && momEv.direction !== 'NEUTRAL') {
    contradictions.push(`Struktur trend ${trendEv.direction === 'BULLISH' ? 'bullish' : 'bearish'} tapi momentum RSI-14 menunjukkan arah berlawanan (${momEv.detail}).`);
  }
  ...
  return {
    ...
    confluenceScore, confidenceScore, dataQuality, rrToTp2, computedAt: new Date().toISOString()
  };
}
...
async function generateTradingHypothesis(ticker) {
  const confluence = await computeConfluence(ticker);
  const { signal, regime, evidence, contradictions, missingEvidence, confluenceScore, confidenceScore, dataQuality, rrToTp2 } = confluence;
  ...
  // Illustrative sizing only, on the same Rp100jt/1%-risk notional the
  // isolated paper account starts with — the client recomputes REAL sizing
  // against the user's actual live paper-account equity/cash the moment a
  // position is actually opened (aiOpenPositionFromSignal), exactly like
  // every other signal already flowing through that function.
  const REFERENCE_ACCOUNT_RP = 100000000;
  const REFERENCE_RISK_PCT = 1;
  let suggestedPositionSizing = null;
  if (!noTrade && signal.entry != null && signal.sl != null) {
    const riskPerShare = signal.entry - signal.sl;
    const riskBudget = REFERENCE_ACCOUNT_RP * (REFERENCE_RISK_PCT / 100);
    const shares = riskPerShare > 0 ? Math.floor(riskBudget / riskPerShare / 100) * 100 : 0;
    suggestedPositionSizing = {
      referenceAccountRp: REFERENCE_ACCOUNT_RP,
      riskPct: REFERENCE_RISK_PCT,
      riskBudgetRp: Math.round(riskBudget),
      riskPerShareRp: Math.round(riskPerShare),
      suggestedShares: shares,
      suggestedLots: Math.floor(shares / 100),
      note: 'Ilustratif pada akun referensi Rp100 juta — ukuran posisi riil dihitung ulang otomatis terhadap saldo & risiko akun paper Anda saat "Buka Posisi Paper" ditekan.'
    };
  }
...
async function warmRadarFundamentals(tickers) {
  const todo = tickers.filter(t => !_radarWarmInFlight.has(t) && getCachedFundamentalsOnly(t) === null && !_fundamentalsCache.has(t));
...
async function getUniverseAccumulationDistribution(params = {}) {
  ...
  if (!apiKey) {
    return {
      ...
      timeframe: tf,
      counts: { accumulation: 0, distribution: 0, totalUniverseScanned: Object.keys(universe).length },
      accumulation: [],
      distribution: [],
  ...
  results.forEach((r) => {
    ...
    if (summary.isSimulated) return; // never mix simulated entries into a "scan result"
    anyReal = true;
