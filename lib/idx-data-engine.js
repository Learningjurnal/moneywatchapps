...
function brokerSummaryDateRange(timeframe) {
  ...
  // toDateDisplay stays dashed (YYYY-MM-DD), matching every other
  // reportDate in this file (e.g. line ~1501/1634) — fromDate/toDate stay
  // compact (YYYYMMDD) since that's the query-param format
  // fetchInvezgoBrokerSummary()/the cache key already expect.
  return { fromDate: compact(from), toDate: compact(to), toDateDisplay: to.toISOString().slice(0, 10) };
}
...
//
// FIX AUDIT (2026-09-17, user-requested: "harus mampu screener seluruh
// saham di BEI, ... jangan batasi hanya pada indeks LQ45/IDX80/Kompas"):
// this used to hardcode the LQ45 constituents (45 tickers) as the ONLY
// possible scan universe, no matter what caller asked for — the function
// signature took no ticker list at all. That made "Universe-Wide"/"seluruh
// BEI" in the UI/docs factually false (confirmed by INV-007 above). Now
// accepts an explicit `params.tickers` list from the caller (same pattern
// as POST /api/idx/ai-scan, which already lets the client scan any
// universe including the full ~900+ IDX list in batches) — LQ45 is only
// the DEFAULT when no list is given, not the ceiling. A ticker with no
// usable price/broker data is skipped (see the quote/summary checks
// below), never fabricated, so "keterbatasan data" empties that row
...
async function getUniverseAccumulationDistribution(params = {}) {
  ...
  // whatever ticker list the caller asked for (LQ45 by default when none
  // is given — see FIX AUDIT above for why LQ45 is no longer the hard
  // ceiling) through the real per-ticker broker-flow pipeline and
  // classifies by its real verdict.
  // NOTE: this fires up to BATCH_CAP generateBrokerSummary() calls "at
  // once" via Promise.allSettled below, but each one goes through
  ...
  // it's safe to write this as a naive-looking Promise.allSettled. The cap
  // itself exists because each call spends real Invezgo quota (30k/month
  // by default) and a serverless request has a hard wall-clock limit —
  // scanning the full ~900+ IDX universe is done by the CALLER issuing
  // several capped requests in sequence (see fsRenderBrokerFlowMode() in
  // public/js/07-flowscan.js), the same batching pattern already used by
  // POST /api/idx/ai-scan for large universes.
  const BATCH_CAP = 80;
  let tickers;
  if (Array.isArray(params.tickers) && params.tickers.length) {
    tickers = Array.from(new Set(params.tickers.map(t => String(t).toUpperCase().replace(/\.JK$/i, '').trim()))).slice(0, BATCH_CAP);
  } else if (typeof params.tickers === 'string' && params.tickers.trim()) {
    tickers = Array.from(new Set(params.tickers.split(',').map(t => t.toUpperCase().replace(/\.JK$/i, '').trim()).filter(Boolean))).slice(0, BATCH_CAP);
  } else {
    tickers = Object.values(universe).filter(u => u.indexes && u.indexes.lq45).map(u => u.code).slice(0, BATCH_CAP);
  }
  ...
  // ticker IDX universe — even though only the (LQ45-capped) `tickers`
  // above were actually queried. That's a real, verifiable inaccuracy: it
  // claimed to have scanned far more of the market than it did. Report the
  // actual scanned count instead, whatever list was actually passed in.
  ...
  if (!anyReal) {
    return {
      success: true,
      isSimulated: true,
      dataSource: 'Invezgo dikonfigurasi tapi tidak ada respons real untuk emiten manapun di batch ini',
      timeframe: tf,
