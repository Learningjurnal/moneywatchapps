...
async function runUnifiedScreenerBacktest(params = {}) {
  ...
  perDateResults.forEach((r) => {
    ...
    accByCode.forEach((accRow, ticker) => {
      ...
      let whaleScore = 2; // accumulation match (same weight as generateUnifiedScreener())
      if (netBuyByCode.has(ticker)) whaleScore += 1;
      if (tech.volRatio >= 2 && tech.trend === 'UPTREND') whaleScore += 1;

      const uptrendScoreTechOnly = tech.score; // valuation deliberately excluded, see file header comment
      const confirmed = whaleScore >= 3 && uptrendScoreTechOnly >= 60;
      if (!confirmed) return;
      ...
      signals.push({
        ticker, date: r.date, whaleScore, uptrendScoreTechOnly,
        entryPrice: Math.round(entryPrice), exitPrice: Math.round(exitPrice),
