...
async function generateUnifiedScreener(params = {}) {
  const universe = loadBaseUniverse();
  const allList = Object.values(universe);
  const { search, sector, index, whale, minUptrend, maxPer, minRoe, confirmedOnly, wavePhase, sort, order, limit, offset } = params;

  const [accDist, foreignFlow] = await Promise.all([
  ...
  const evaluated = allList.map((stock) => {
    ...
    const confirmed = uptrendScore != null && uptrendScore >= 60 && whaleScore >= 3;
    ...
    return {
      ...
      whaleSignals,
      whaleDataAvailable,
      confirmedUptrendWhale: confirmed,
...
async function runUnifiedScreenerBacktest(params = {}) {
  ...
  const histories = await Promise.allSettled(candidates.map((t) => fetchYahooHistory(t, 'BACKTEST')));
  ...
  const baselineFilter = (s) => s.whaleScoreFull >= 3 && s.techScore >= 60;

  // ?variants=true (2026-09-19, user-requested: test 3 formula tweaks found
  ...
  // Invezgo/Yahoo data) for all 4 comparisons, no extra network calls:
  //   baseline        = current production formula, unchanged
  ...
  //   techScore80     = stricter technical conviction gate (>=80 instead of >=60)
  if (params.variants === true || params.variants === 'true') {
    return {
      success: true,
      available: true,
      methodology: 'Perbandingan 4 varian formula whale+teknikal dari POOL SINYAL YANG SAMA (data Invezgo/Yahoo di-fetch sekali, tidak diulang per varian) — baseline (formula produksi saat ini), whale4 (ambang whaleScore>=4), noDoubleCount (bonus volume-spike+uptrend dihapus dari whaleScore karena sudah dihitung juga di tech.score), techScore80 (ambang skor teknikal>=80). Komponen VALUASI (PER/ROE) SENGAJA TIDAK disertakan di semua varian — lihat komentar header fungsi ini.',
      lookbackDays,
      forwardDays,
      datesScanned: dates.length,
      candidatesScanned: candidates.length,
      variants: {
        baseline: summarize(candidatePool, baselineFilter, 'Baseline (whaleScore>=3, techScore>=60) — formula produksi saat ini'),
        whale4: summarize(candidatePool, (s) => s.whaleScoreFull >= 4 && s.techScore >= 60, 'whaleScore>=4 (ambang whale lebih ketat)'),
        noDoubleCount: summarize(candidatePool, (s) => s.whaleScoreNoVolSpike >= 3 && s.techScore >= 60, 'Bonus volume-spike+uptrend dihapus dari whaleScore (anti dobel-hitung)'),
        techScore80: summarize(candidatePool, (s) => s.whaleScoreFull >= 3 && s.techScore >= 80, 'techScore>=80 (ambang teknikal lebih ketat)')
      },
      computedAt: new Date().toISOString()
    };
  }
...
async function resolveScreenerSignalLog() {
  ...
  const [histories, ihsgHist] = await Promise.all([
    Promise.allSettled(tickers.map((t) => fetchYahooHistory(t, 'BACKTEST'))),
