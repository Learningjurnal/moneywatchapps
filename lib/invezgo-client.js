...
async function storeSetEx(key, value, ttlSec) {
  const redis = getRedis();
  // FIX (2026-09-24, real production incident — GitHub Actions run
  // 35964062571): Upstash/Redis rejects `EX 0` with "ERR invalid expire
  // time" — passing {ex: ttlSec} unconditionally broke the very first
  // caller that used ttlSec=0 to mean "no expiry" (warmStrategyEngineRotating()'s
  // cursor key, lib/engine/strategy/StrategyEngine.js). Same fix as the
  // already-correct yfStoreSetEx() in lib/providers/yahoo-client.js: only
  // pass the ex option when ttlSec is truthy.
 ...
 */
async function getOrFetch(endpoint, params, ttlSec, fetcher, forceRefresh = false) {
...
// data"): NONE of this app's outbound fetch() calls (Yahoo/idx.co.id/
// Invezgo) had a timeout. A silently-hanging third-party response (not an
// error, just no reply) left the request stuck until Vercel's hard 30s
// function ceiling killed it — no clean error, just a spinner that never
// resolves. invezgoFetch() is the single chokepoint for ~15 Invezgo
// endpoints, so fixing it here covers all of them at once. A timeout
// throws (AbortError), which every fetchInvezgo*() caller already catches
// in its own try/catch and reports as NETWORK_ERROR — no new error-
// handling needed anywhere else.
const INVEZGO_FETCH_TIMEOUT_MS = Number(process.env.INVEZGO_FETCH_TIMEOUT_MS || 10000);
...
async function invezgoFetch(url, options = {}) {
  ...
  try {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      const resp = await fetch(url, { ...finalOptions, signal: AbortSignal.timeout(INVEZGO_FETCH_TIMEOUT_MS) });
      lastResp = resp;
...
async function fetchInvezgoBrokerSummary(ticker, fromDate, toDate, investor = 'all', market = 'RG', forceRefresh = false) {
  ...
  const effectiveInvestor = String(investor || 'all').toLowerCase();
  const effectiveMarket = String(market || 'RG').toUpperCase();

  const params = { ticker, fromDate, toDate, investor: effectiveInvestor, market: effectiveMarket };
  const envelope = await getOrFetch('broker_summary', params, INVEZGO_BROKER_SUMMARY_CACHE_TTL_SEC, async () => {
    try {
      const url = `${INVEZGO_BASE_URL}/analysis/summary/stock/${encodeURIComponent(ticker)}?from=${fromDate}&to=${toDate}&investor=${encodeURIComponent(effectiveInvestor)}&market=${encodeURIComponent(effectiveMarket)}`;
...
// ── Invezgo API integration (Strategy Engine V1 data sources, 2026-09-24) ──
// Confirmed from the user-supplied Invezgo OpenAPI spec (api-1.yaml,
// operationId AnalysisController_getOrderBook /
// BatchController_getOrderBookBatch / AnalysisController_getIntradayData —
// not a guessed schema).
//
// BEI_MARKET_SEGMENT — the `market` query param (RG/NG/TN) required by both
// endpoints below. IMPORTANT PROVENANCE NOTE: Invezgo's own OpenAPI spec
// (13,822 lines, grepped in full) NEVER spells out what RG/NG/TN stand for
// — it only ever appears as a bare enum. The mapping below is NOT confirmed
// from Invezgo's documentation; it is BEI's own official market-segment
// terminology (Peraturan Perdagangan BEI No. II-A: Pasar Reguler / Pasar
// Negosiasi / Pasar Tunai), which Invezgo appears to pass through verbatim
// (its `running-trade` endpoint's example response literally tags a trade
// `board: RG`, matching BEI's own segment coding). Confidence: medium-high,
// industry-standard terminology — but NOT independently verified against a
// live Invezgo response (the user cannot reach api.invezgo.com from here to
// confirm behaviorally). Treat any indicator built on this mapping
// (HIGH_NON_REGULAR) as provisional until a live test call confirms it.
const BEI_MARKET_SEGMENT = Object.freeze({
  RG: { code: 'RG', name: 'Pasar Reguler', isRegular: true },
  NG: { code: 'NG', name: 'Pasar Negosiasi', isRegular: false },
  TN: { code: 'TN', name: 'Pasar Tunai', isRegular: false }
});

const INVEZGO_ORDER_BOOK_CACHE_TTL_SEC = Number(process.env.INVEZGO_ORDER_BOOK_CACHE_TTL_SEC || 60);
// The OpenAPI spec's /batch/order-book/{code} description says "10 kode
// untuk MAX, 25 kode untuk ELITE" — it never mentions an "Advance" plan
// tier (the plan the user has) for this specific batch limit. Defaulting
// to the SMALLER documented limit (10) is the safe assumption until
// confirmed otherwise; override via env var if Invezgo support confirms a
// higher limit for Advance.
const INVEZGO_ORDER_BOOK_BATCH_SIZE = Number(process.env.INVEZGO_ORDER_BOOK_BATCH_SIZE || 10);

function normalizeMarketParam(market) {
  const m = String(market || '').toUpperCase().trim();
  return BEI_MARKET_SEGMENT[m] ? m : null;
}

/**
 * fetchInvezgoOrderBook(code, market, date, time) — GET /analysis/order-book/{code}
 * Real top-of-book (LEVEL 1 ONLY — Invezgo does not expose deeper levels;
 * grepped the whole spec for bid2/bid3/.../offer5, zero matches) bid/offer.
 * Response shape (from a real, vendor-captured "Test Request" example):
 *   { code, bid: [{bid1price, bid1lot, bid1freq}], offer: [{offer1price, offer1lot, offer1freq}] }
 * 204 = no order book (stock likely suspended) — returns bid:[]/offer:[].
 */
async function fetchInvezgoOrderBook(code, market, date, time) {
  const apiKey = process.env.INVEZGO_API_KEY;
  if (!apiKey) return { ok: false, reason: 'NOT_CONFIGURED' };
  const cleanCode = String(code || '').trim().toUpperCase();
  const seg = normalizeMarketParam(market);
  if (!cleanCode || !seg) return { ok: false, reason: 'INVALID_PARAMS' };

  const params = { code: cleanCode, market: seg, date: date || '', time: time || '' };
  const envelope = await getOrFetch('order_book', params, INVEZGO_ORDER_BOOK_CACHE_TTL_SEC, async () => {
    try {
      const qs = new URLSearchParams({ market: seg });
      if (date) qs.set('date', date);
      if (time) qs.set('time', time);
      const url = `${INVEZGO_BASE_URL}/analysis/order-book/${encodeURIComponent(cleanCode)}?${qs.toString()}`;
      ...
      if (resp.status === 402) return { status: 'UNAVAILABLE', reason: 'SUBSCRIPTION_INSUFFICIENT', provider: 'Invezgo', retrievedAt, requestId };
      if (resp.status === 429) return { status: 'UNAVAILABLE', reason: 'RATE_LIMITED', provider: 'Invezgo', retrievedAt, requestId };
      if (resp.status === 204) return { status: 'REAL', source: 'invezgo_api', provider: 'Invezgo', retrievedAt, dataTimestamp: retrievedAt, freshnessSeconds: 0, requestId, suspended: true, bid: [], offer: [] };
      if (!resp.ok) return { status: 'UNAVAILABLE', reason: `HTTP_${resp.status}`, provider: 'Invezgo', retrievedAt, requestId };

      const raw = await resp.json();
      if (!raw || !Array.isArray(raw.bid) || !Array.isArray(raw.offer)) {
        return { status: 'INVALID', reason: 'UNEXPECTED_SCHEMA', provider: 'Invezgo', retrievedAt, requestId, raw };
      }
      return { status: 'REAL', source: 'invezgo_api', provider: 'Invezgo', retrievedAt, dataTimestamp: retrievedAt, freshnessSeconds: date ? null : 0, requestId, suspended: false, bid: raw.bid, offer: raw.offer };
    } catch (e) {
      return { status: 'UNAVAILABLE', reason: 'NETWORK_ERROR', provider: 'Invezgo', retrievedAt: new Date().toISOString(), requestId: makeRequestId(), error: e.message };
    }
  });
  ...
  if (envelope.status === 'REAL') {
    return { ok: true, source: 'invezgo_real', suspended: !!envelope.suspended, bid: envelope.bid, offer: envelope.offer, quality: envelope };
  }
  return { ok: false, reason: envelope.reason || envelope.status, quality: envelope };
}

/**
 * fetchInvezgoOrderBookBatch(codes[], market, date, time) —
 * GET /batch/order-book/{code} (pipe-separated codes). Splits into chunks
 * of INVEZGO_ORDER_BOOK_BATCH_SIZE automatically — caller does not need to
 * chunk manually. Returns a map keyed by ticker code.
 */
async function fetchInvezgoOrderBookBatch(codes, market, date, time) {
  const apiKey = process.env.INVEZGO_API_KEY;
  if (!apiKey) return { ok: false, reason: 'NOT_CONFIGURED' };
  const seg = normalizeMarketParam(market);
  const list = (Array.isArray(codes) ? codes : []).map(c => String(c || '').trim().toUpperCase()).filter(Boolean);
  if (!list.length || !seg) return { ok: false, reason: 'INVALID_PARAMS' };

  const byTicker = {};
  const errors = [];
  for (let i = 0; i < list.length; i += INVEZGO_ORDER_BOOK_BATCH_SIZE) {
    const chunk = list.slice(i, i + INVEZGO_ORDER_BOOK_BATCH_SIZE);
    const params = { codes: chunk.join('|'), market: seg, date: date || '', time: time || '' };
    const envelope = await getOrFetch('order_book_batch', params, INVEZGO_ORDER_BOOK_CACHE_TTL_SEC, async () => {
      try {
        const qs = new URLSearchParams({ market: seg });
        if (date) qs.set('date', date);
        if (time) qs.set('time', time);
        const url = `${INVEZGO_BASE_URL}/batch/order-book/${encodeURIComponent(chunk.join('|'))}?${qs.toString()}`;
        const resp = await invezgoFetch(url, { headers: { 'Authorization': `Bearer ${apiKey}`, 'Accept': 'application/json' } });
        const requestId = makeRequestId();
        const retrievedAt = new Date().toISOString();
        ...
        const raw = await resp.json();
        if (!Array.isArray(raw)) return { status: 'INVALID', reason: 'UNEXPECTED_SCHEMA', provider: 'Invezgo', retrievedAt, requestId, raw };
        return { status: 'REAL', source: 'invezgo_api', provider: 'Invezgo', retrievedAt, dataTimestamp: retrievedAt, freshnessSeconds: 0, requestId, rows: raw };
      } catch (e) {
        return { status: 'UNAVAILABLE', reason: 'NETWORK_ERROR', provider: 'Invezgo', retrievedAt: new Date().toISOString(), requestId: makeRequestId(), error: e.message };
      }
    });
    ...
    if (envelope.status === 'REAL') {
      (envelope.rows || []).forEach(row => {
        if (row && row.code) byTicker[row.code] = { bid: row.bid || [], offer: row.offer || [] };
      });
    } else {
      errors.push({ chunk, reason: envelope.reason || envelope.status });
    }
  }

  return { ok: errors.length < Math.ceil(list.length / INVEZGO_ORDER_BOOK_BATCH_SIZE), byTicker, errors };
}

const INVEZGO_INTRADAY_CACHE_TTL_SEC = Number(process.env.INVEZGO_INTRADAY_CACHE_TTL_SEC || 60);

/**
 * fetchInvezgoIntradayData(code, market, date) — GET /analysis/intraday-data/{code}
 * Real per-market-segment daily aggregate: open/high/low/close/avg/volume/
 * freq/value/prev + top-of-book bid_price/bid_lot/bid_freq/offer_price/
 * offer_lot/offer_freq. `freq` is REAL total trade-count for the day (not a
 * formula estimate) — this is the source for HIGH_ATS. `market` is
 * REQUIRED (no "ALL" option on this endpoint per the spec) — computing
 * HIGH_NON_REGULAR therefore needs 3 separate calls (RG+NG+TN) per ticker,
 * not 1.
 */
async function fetchInvezgoIntradayData(code, market, date) {
  const apiKey = process.env.INVEZGO_API_KEY;
  if (!apiKey) return { ok: false, reason: 'NOT_CONFIGURED' };
  const cleanCode = String(code || '').trim().toUpperCase();
  const seg = normalizeMarketParam(market);
  ...
  const params = { code: cleanCode, market: seg, date: date || '' };
  const envelope = await getOrFetch('intraday_data', params, INVEZGO_INTRADAY_CACHE_TTL_SEC, async () => {
    try {
      const qs = new URLSearchParams({ market: seg });
      if (date) qs.set('date', date);
      const url = `${INVEZGO_BASE_URL}/analysis/intraday-data/${encodeURIComponent(cleanCode)}?${qs.toString()}`;
      const resp = await invezgoFetch(url, { headers: { 'Authorization': `Bearer ${apiKey}`, 'Accept': 'application/json' } });
      const requestId = makeRequestId();
      const retrievedAt = new Date().toISOString();
      ...
      if (resp.status === 402) return { status: 'UNAVAILABLE', reason: 'SUBSCRIPTION_INSUFFICIENT', provider: 'Invezgo', retrievedAt, requestId };
      if (resp.status === 429) return { status: 'UNAVAILABLE', reason: 'RATE_LIMITED', provider: 'Invezgo', retrievedAt, requestId };
      if (resp.status === 204) return { status: 'REAL', source: 'invezgo_api', provider: 'Invezgo', retrievedAt, dataTimestamp: retrievedAt, freshnessSeconds: 0, requestId, suspended: true, data: null };
      if (!resp.ok) return { status: 'UNAVAILABLE', reason: `HTTP_${resp.status}`, provider: 'Invezgo', retrievedAt, requestId };

      const raw = await resp.json();
      if (!raw || typeof raw.volume !== 'number' || typeof raw.freq !== 'number') {
        return { status: 'INVALID', reason: 'UNEXPECTED_SCHEMA', provider: 'Invezgo', retrievedAt, requestId, raw };
      }
      return { status: 'REAL', source: 'invezgo_api', provider: 'Invezgo', retrievedAt, dataTimestamp: retrievedAt, freshnessSeconds: date ? null : 0, requestId, suspended: false, data: raw };
    } catch (e) {
      return { status: 'UNAVAILABLE', reason: 'NETWORK_ERROR', provider: 'Invezgo', retrievedAt: new Date().toISOString(), requestId: makeRequestId(), error: e.message };
    }
  });
  ...
  if (envelope.status === 'REAL') {
    return { ok: true, source: 'invezgo_real', suspended: !!envelope.suspended, data: envelope.data, quality: envelope };
  }
  return { ok: false, reason: envelope.reason || envelope.status, quality: envelope };
}

// ── Invezgo API integration (Stock Notation List — Regulatory Health ──
// Gate primary source, 2026-09-24) ──
// FIX (user-reported: "Regulatory Health Gate cek apakah data ini bisa di
// dapatkan, atau data tidak pernah tersedia, kalo data tidak pernah
// tersedia, screener akan terus 0"): the gate's only source used to be
// fetchIdxSpecialNotations() (lib/providers/idx-client.js), which scrapes
// idx.co.id's internal AJAX endpoints directly — reachability from a
// Vercel serverless IP was never verified and idx.co.id may well block
// datacenter/bot traffic (common for exchange sites), which would leave
// the gate permanently DATA_ERROR and the whole Screener permanently
// empty. GET /analysis/notation (confirmed via the user-supplied Invezgo
// OpenAPI spec, operationId AnalysisController_notation) is Invezgo's own
// official notation endpoint — 1 call, whole-market, no parameters — and
// Invezgo is already PROVEN reachable from this app's Vercel deployment
// (the Strategy Engine cron's first live run processed 843/958 tickers
// against it). User decision (2026-09-24): use this as the PRIMARY
// regulatory-data source, keep fetchIdxSpecialNotations() as fallback —
// see getRegulatoryHealthGate() in lib/regulatory-gate.js for how the two
// are combined.
//
// PROVENANCE CAVEAT: the spec's example shows one {code, date, list} entry
// per stock; it is NOT confirmed from a live response whether a stock can
// have MULTIPLE {date, list} entries (a real notation history) or always
// exactly one (current status only). Callers must not assume either —
// lib/regulatory-gate.js's normalizer takes the entry with the latest
// `date` per code as "current" and discloses this as an assumption.
const INVEZGO_NOTATION_CACHE_TTL_SEC = Number(process.env.INVEZGO_NOTATION_CACHE_TTL_SEC || 43200); // 12h, matches the old idx.co.id cadence

async function fetchInvezgoNotation(forceRefresh = false) {
  const apiKey = process.env.INVEZGO_API_KEY;
  if (!apiKey) return { ok: false, reason: 'NOT_CONFIGURED' };

  const envelope = await getOrFetch('notation', {}, INVEZGO_NOTATION_CACHE_TTL_SEC, async () => {
    try {
      const url = `${INVEZGO_BASE_URL}/analysis/notation`;
      const resp = await invezgoFetch(url, { headers: { 'Authorization': `Bearer ${apiKey}`, 'Accept': 'application/json' } });
      const requestId = makeRequestId();
      const retrievedAt = new Date().toISOString();
  ...
  if (envelope.status === 'REAL') {
    return { ok: true, source: 'invezgo_real', rows: envelope.rows, quality: envelope };
  }
  return { ok: false, reason: envelope.reason || envelope.status, quality: envelope };
}
...
export {
  ...
  fetchInvezgoOrderBook,
  fetchInvezgoOrderBookBatch,
  fetchInvezgoIntradayData,
  fetchInvezgoNotation,
  BEI_MARKET_SEGMENT,
  MONTHLY_QUOTA,
  MAX_CONCURRENCY,
