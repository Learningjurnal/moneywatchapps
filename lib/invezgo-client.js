...
// of TTL — but it DOES mean two full scans run more than 5 minutes apart
// (by the same user re-scanning, or two different users/sessions the same
// day) each cost a fresh ~958 units of the shared 30,000/month budget,
// even though broker-flow composition for a given trading day doesn't
// meaningfully change minute-to-minute for a retail screening tool (unlike
// a live day-trading terminal). Bumped to 30 minutes by default — still
// intraday-fresh for the single-ticker StockChat/Bandarmology lookups that
// share this same cache, but now a second full-BEI scan (or repeat
// per-ticker StockChat lookups) within the same half-hour is nearly free
// instead of nearly full price. Configurable via env var so the operator
// can tune the freshness/budget tradeoff without a code change.
const INVEZGO_BROKER_SUMMARY_CACHE_TTL_SEC = Number(process.env.INVEZGO_BROKER_SUMMARY_CACHE_TTL_SEC || 1800);
