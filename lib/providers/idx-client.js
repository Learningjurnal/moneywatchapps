...
// data"): none of this file's fetch() calls to idx.co.id had a timeout —
// a silently-hanging response left the request stuck until Vercel's 30s
// function ceiling killed it with no clean error. Every caller already
// catches failures and degrades honestly (or, for the Regulatory Health
// Gate, falls back to Invezgo — see lib/regulatory-gate.js), so a
// timeout-triggered AbortError needs no new handling.
const IDX_FETCH_TIMEOUT_MS = Number(process.env.IDX_FETCH_TIMEOUT_MS || 8000);
...
async function ensureIdxSession() {
  ...
  _idxSessionPromise = (async () => {
    const resp = await fetch('https://www.idx.co.id/id', { headers: IDX_BROWSER_HEADERS, signal: AbortSignal.timeout(IDX_FETCH_TIMEOUT_MS) });
    ...
    try {
      await fetch('https://www.idx.co.id/primary/home/GetIndexList', {
        ...
        signal: AbortSignal.timeout(IDX_FETCH_TIMEOUT_MS)
      });
...
// FIX (Regulatory Health Gate, 2026-09-24): sebelumnya fungsi ini SELALU
// mengembalikan bare `{ticker: {...}}` map -- kalau kedua panggilan
// idx.co.id gagal (403/500/network error) TANPA cache lama, hasilnya map
// KOSONG `{}`, yang secara visual TIDAK BISA dibedakan dari "sudah dicek,
// benar-benar 0 emiten bernotasi". Konsumen (gate/UI) tidak bisa tahu
// bedanya "AMAN (CLEAR)" vs "TIDAK DIKETAHUI (fetch gagal)" -- pelanggaran
// langsung terhadap docs/regulatory-health-gate.md §7 ("UNKNOWN -> NOT
// ELIGIBLE, never UNKNOWN -> CLEAR"). Sekarang mengembalikan AMPLOP
// eksplisit {available, isStale, checkedAt, byTicker} -- `available`
// hanya true kalau MINIMAL SATU dari 2 panggilan idx.co.id berhasil (.ok)
// pada percobaan ini ATAU cache masih segar; `isStale` true kalau data
// yang dikembalikan berasal dari cache yang sudah lewat TTL karena
// percobaan refresh gagal total.
let _specialNotationCache = null; // { byTicker, checkedAt }
const NOTATION_CACHE_TTL_MS = 12 * 60 * 60 * 1000;

async function fetchIdxSpecialNotations(forceRefresh = false) {
  const now = Date.now();
  if (!forceRefresh && _specialNotationCache && (now - _specialNotationCache.checkedAt) < NOTATION_CACHE_TTL_MS) {
    return { available: true, isStale: false, checkedAt: new Date(_specialNotationCache.checkedAt).toISOString(), byTicker: _specialNotationCache.byTicker };
  }
  ...
  try {
    ...
    if (anyFetchSucceeded) {
      _specialNotationCache = { byTicker, checkedAt: now };
      console.log(`[IDX Notasi Khusus] ${Object.keys(byTicker).length} emiten bernotasi khusus berhasil disinkronkan dari idx.co.id.`);
      return { available: true, isStale: false, checkedAt: new Date(now).toISOString(), byTicker };
    }

    // Kedua panggilan idx.co.id gagal (bukan exception, cuma !ok) -- JANGAN
    // diam-diam anggap "0 notasi" (itu klaim CLEAR yang tidak terverifikasi).
    console.warn('[IDX Notasi Khusus] Kedua endpoint idx.co.id gagal (bukan 200 OK), pakai cache lama kalau ada.');
    if (_specialNotationCache) {
      return { available: false, isStale: true, checkedAt: new Date(_specialNotationCache.checkedAt).toISOString(), byTicker: _specialNotationCache.byTicker, reason: 'IDX_FETCH_FAILED' };
    }
    return { available: false, isStale: false, checkedAt: null, byTicker: {}, reason: 'IDX_FETCH_FAILED' };
  } catch (e) {
    console.warn('[IDX Notasi Khusus] Fetch gagal, pakai cache lama kalau ada:', e.message);
    if (_specialNotationCache) {
      return { available: false, isStale: true, checkedAt: new Date(_specialNotationCache.checkedAt).toISOString(), byTicker: _specialNotationCache.byTicker, reason: e.message || 'NETWORK_ERROR' };
    }
    return { available: false, isStale: false, checkedAt: null, byTicker: {}, reason: e.message || 'NETWORK_ERROR' };
  }
}
