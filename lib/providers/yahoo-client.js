...
// data"): none of this file's fetch() calls to finance.yahoo.com had a
// timeout — a silently-hanging response left the request stuck until
// Vercel's 30s function ceiling killed it with no clean error. Every
// caller of these functions already wraps them in try/catch and degrades
// honestly on failure, so a timeout-triggered AbortError needs no new
// error handling — it just fails fast instead of hanging.
const YAHOO_FETCH_TIMEOUT_MS = Number(process.env.YAHOO_FETCH_TIMEOUT_MS || 8000);
