...
function aiRunGemini(){
  ...
  var payload = {
    portfolioSummary: {
      totalMV: ctx.m.totalMV,
      totalCost: ctx.m.totalCost,
      totalReturn: ctx.m.totalReturn,
      real: ctx.m.real,
      unreal: ctx.m.unreal,
      n: ctx.m.n,
      cashPct: ctx.cashPct
    ...
    metrics: {
      beta: ctx.m.beta,
      topSec: ctx.m.topSec,
      topSecPct: ctx.m.topSecPct,
      var95: ctx.m.var95
    },
    hfMetrics: ctx.hf,
    ihsg: ctx.ihsg + (ctx.ihsgChg ? ' (' + ctx.ihsgChg + ')' : '')
  };
